...
class ModelLSTM(torch.nn.Module):
	def __init__(self):
		...
		self.conv3 = nn.Conv2d(20,20,kernel_size=5, padding=2,stride=1)
		self.relu3 = nn.ReLU()
		
		self.fc1 = nn.Linear(64*64*20, 1000)
		self.relu4 = nn.ReLU()
...
def validate(model, dataloaderValid):
	...
	#read the input and the fitting target into the device
				...
				seqLen = x.shape[1]
				inputSize = (x.shape[2],x.shape[3])
				outputSize = y.shape[2]
				...
				for f in range(seqLen):
					...
					# loss per frame
					validloss += lossfunc(result, target[f,:,:])

				# Compute loss per batch
				validationLoss = validloss / seqLen
				epochValidloss += validationLoss

	# this is used to set a description in the tqdm progress bar 
	t.set_description(f"validation loss {epochValidloss}")
	model.train()
	return epochValidloss
	...
	for epoc in range(100):
		...
		with trange(len(dataloaderTrain)) as t:
			for idx in t:
				...
				if idx == 0:
					lossMovingAveraged = trainloss
				else:
					lossMovingAveraged = 0.99*lossMovingAveraged + (1-0.99)*trainloss
